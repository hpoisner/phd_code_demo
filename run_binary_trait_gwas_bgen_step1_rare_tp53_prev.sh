...
bed_file="project-J14qfbjJ21Qq0q2X50kBgXzb:file-J38qQ7QJ21Qy69P38qJK3jFJ"
bim_file="project-J14qfbjJ21Qq0q2X50kBgXzb:file-J38qQ7QJ21QXfB1vFG83fF76"
fam_file="project-J14qfbjJ21Qq0q2X50kBgXzb:file-J38qQ7QJ21Qb9b1g77FvJQzz"
...
tsv_file="rare_files_single_chrom_rcv.txt"

# Dynamically set input file names
pheno_file_name=$(dx ls "${pheno_file}")
covar_file_name=$(dx ls "${covar_file}")
bed_file_name=$(dx ls "${bed_file}")
bim_file_name=$(dx ls "${bim_file}")
fam_file_name=$(dx ls "${fam_file}")
...
tail -n +2 "${tsv_file}" | {
    while IFS=$'\t' read -r chr igenotype_bgens igenotype_bgis igenotype_samples istep2_anno_file istep2_gene_list; do
        ...
        bed_file_name_basename=$(basename "${bed_file_name}" | sed 's/\.[^.]*$//')

        # Dynamically set input strings
        step1_outfile="${outfile_name}_pred.list"
        ...
        echo "  - BED File: ${bed_file_name}"
        echo "  - BIM File: ${bim_file_name}"
        echo "  - FAM File: ${fam_file_name}"
